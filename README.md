...
- Weekly automated `develop` → `main` promotion via the `Promote to Main` workflow (Mondays 15:00 JST, squash-merged after required checks pass)
- Post-deploy `Production Smoke Check` verifies the live routes after every push to `main`; recovery is Cloudflare Pages Instant Rollback
